...
public class ByteUtil {
    ...
    public static String getSum16(byte[] msg, int length) {
        ...
        result = byteToStr(msg, length) + byteToStr(mByte, mByte.length).substring(byteToStr(mByte,mByte.length).length() - 4, byteToStr(mByte, mByte.length).length());
        return result;
    }

    public static String getSum162(byte[] msg, int length) {
        long mSum = 0;
        byte[] mByte = new byte[length];
        String result;
        /** 逐Byte添加位数和 */
        ...
        result = byteToStr(msg, length) + byteToStr(mByte, mByte.length).substring(byteToStr(mByte,mByte.length).length() - 2);
        return result;
    }
    ...
    public static byte checksum(byte[] data,int size)
    {
        ...
        System.arraycopy(data, 0,bytes,0, size - 1);
        for (byte item : bytes)
        {
            cks = (cks + item) % 0xffff;
        }
